   ...
   if(isset($_POST['reguser'])){
	...
	$q_checkadmin = $conn->query("SELECT * FROM `login_user` WHERE `email_address` = '$email_address'") or die(mysqli_error());
		...
		if($v_checkadmin == 1){
			...
			$conn->query("INSERT INTO `login_user` (name, email_address, user_password, user_status) VALUES('$user_name', '$email_address', '$user_password', '$user_status')") or die(mysqli_error());
			echo '
				<script type = "text/javascript">
